...
public class BehaviorMecanim : MonoBehaviour
{
	...
	public Node Node_LyingDown()
	{
		return new LeafInvoke(
			() => this.Character.LyingDown());
	}
	
	public Node Node_StandUp_Sleep()
	{
		return new LeafInvoke(
			() => this.Character.StandUp_Sleep());
	}
	...
	public Node ST_TurnToBackX(Val<Vector3> target)
	{
		Vector3 position = target.Value;
		position.Set (-position.x,position.y,position.z);
		target = Val.V (() => position);
		Func<RunStatus> turn =
			() => this.Character.NavTurn(target);
	...
	public Node ST_TurnToBackZ(Val<Vector3> target)
	{
		Vector3 position = target.Value;
		position.Set (position.x,position.y,position.z-5.0f);
		target = Val.V (() => position);
		Func<RunStatus> turn =
			() => this.Character.NavTurn(target);
